if Config.EnableESXService then
	if Config.MaxInService ~= -1 then
		TriggerEvent('esx_service:activateService', 'bcso', Config.MaxInService)
	end
end

TriggerEvent('esx_phone:registerNumber', 'bcso', TranslateCap('alert_bcso'), true, true)
TriggerEvent('esx_society:registerSociety', 'bcso', 'BCSO', 'society_bcso', 'society_bcso', 'society_bcso', {type = 'public'})

RegisterNetEvent('esx_bcsojob:confiscatePlayerItem')
AddEventHandler('esx_bcsojob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	local source = source
...
AddEventHandler('esx_bcsojob:handcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'bcso' then
		TriggerClientEvent('esx_bcsojob:handcuff', target)
	else
...
AddEventHandler('esx_bcsojob:drag', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'bcso' then
		TriggerClientEvent('esx_bcsojob:drag', target, source)
	else
...
AddEventHandler('esx_bcsojob:putInVehicle', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'bcso' then
		TriggerClientEvent('esx_bcsojob:putInVehicle', target)
	else
...
AddEventHandler('esx_bcsojob:OutVehicle', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'bcso' then
		TriggerClientEvent('esx_bcsojob:OutVehicle', target)
	else
		print(('[^3WARNING^7] Player ^5%s^7 Attempted To Exploit Dragging Out Of Vehicle!'):format(xPlayer.source))
	end
end)

RegisterNetEvent('esx_bcsojob:getStockItem')
AddEventHandler('esx_bcsojob:getStockItem', function(itemName, count)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('esx_bcsojob:putStockItems')
AddEventHandler('esx_bcsojob:putStockItems', function(itemName, count)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bcsojob:getOtherPlayerData', function(source, cb, target, notify)
...
ESX.RegisterServerCallback('esx_bcsojob:getFineList', function(source, cb, category)
...
ESX.RegisterServerCallback('esx_bcsojob:getVehicleInfos', function(source, cb, plate)
	local retrivedInfo = {
		plate = plate
	}
	if Config.EnableESXIdentity then
...
ESX.RegisterServerCallback('esx_bcsojob:getArmoryWeapons', function(source, cb)
...
ESX.RegisterServerCallback('esx_bcsojob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bcsojob:removeArmoryWeapon', function(source, cb, weaponName)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bcsojob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bcsojob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bcsojob:storeNearbyVehicle', function(source, cb, plates)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_bcsojob:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_bcso', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('esx_bcsojob:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'bcso' then
			Wait(5000)
			TriggerClientEvent('esx_bcsojob:updateBlip', -1)
		end
	end
end)

RegisterNetEvent('esx_bcsojob:spawned')
AddEventHandler('esx_bcsojob:spawned', function()
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('esx_bcsojob:forceBlip')
AddEventHandler('esx_bcsojob:forceBlip', function()
	for _, xPlayer in pairs(ESX.GetExtendedPlayers('job', 'bcso')) do
		TriggerClientEvent('esx_bcsojob:updateBlip', xPlayer.source)
	end
end)
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_phone:removeNumber', 'bcso')
	end
end)
